...
class ExperimentRestAPI:
    ...
    def api_experiment_get_start_skeleton(self, experiment_id: str) -> Dict[str, Any]:
        ...
        Args:
            experiment_id:
                Unique identifier of the experiment package (as stored on the st4sd-runtime-service
                REST API server)
        ...
        """
        return self.api_request_get(f"/experiments/{experiment_id}/start/payload/")
    ...
    def api_experiment_start(self, experiment_id: str, payload: Dict[str, Any]) -> str:
        ...
        Args:
            experiment_id:
                ...
                REST API server)
            payload:
                Check ST4SD Runtime Service REST-API swagger for the schema of the payload.
                You can also use api_experiment_get_start_skeleton(experiment_id) to generate a skeleton
                for the payload to this experiment.

        Returns:
            rest-uid of the experiment instance
        """
        self.log.info(
    ...
    def api_rest_uid_status(
        ...
        Args:
            rest_uid: rest-uid returned from st4sd-runtime-service API (e.g. api_instance_create(),
    ...
    def api_rest_uid_output(self, rest_uid, output_name, _api_verbose=False) -> Tuple[str, bytes]:
        ...
        """
        self.log.warning("api_rest_uid_output() is deprecated, use api_rest_uid_output_get() instead")
        return self.api_rest_uid_output_get(rest_uid=rest_uid, output_name=output_name, _api_verbose=_api_verbose)

    def api_rest_uid_output_list(self, rest_uid, output_name, _api_verbose=False) -> typing.Dict[
        str, typing.Dict[str, typing.Union[float, str, int]]
    ]:
        """Gets a dictionary describing the key-outputs of this workflow instance
        ...
        Returns
            A dictionary whose keys are names of the key-outputs and values are nested dictionaries
            with the keys "creationtime", "description", "filename", "fielpath", "final", "production",
            "type", "version"
        """
        url = 'instances/%s/outputs/%s' % (rest_uid, output_name)
        response = self.api_request_get(url, return_response=True, _api_verbose=_api_verbose, decode_json=True)

        return response.content

    def api_rest_uid_output_get(self, rest_uid, output_name, _api_verbose=False) -> Tuple[str, bytes]:
        """Gets a key-output that a workflow instance generated
        ...
        try:
            ...
            raise experiment.service.errors.InvalidHTTPRequest(
                ...
                "is st4sd-runtime-service up-to-date ?")
        try:
            ...
            raise experiment.service.errors.InvalidHTTPRequest(
                os.path.join(self.cc_end_point, url), 'GET', response,
                info="Content-Disposition header does not contain a filename is st4sd-runtime-service up-to-date ?")
